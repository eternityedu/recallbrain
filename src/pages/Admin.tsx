...
import { Brain, ArrowLeft, Plus, Trash2, Edit2, Save, X } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-glass-border bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <span className="font-display text-xl font-bold glow-text">Admin Panel</span>
        </div>
      </header>
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="flex justify-between items-center">
            <h1 className="font-display text-2xl font-bold">Pricing Plans</h1>
            <Button variant="glow" onClick={() => setShowNew(true)}><Plus className="h-4 w-4" /> Add Plan</Button>
          </div>
          {showNew && (
            <div className="glass-card p-6 space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div><Label>Name</Label><Input value={newForm.plan_name} onChange={e => setNewForm(p => ({ ...p, plan_name: e.target.value }))} className="input-glass" /></div>
                <div><Label>Price</Label><Input type="number" value={newForm.price} onChange={e => setNewForm(p => ({ ...p, price: +e.target.value }))} className="input-glass" /></div>
                <div><Label>Features (comma-separated)</Label><Input value={newForm.features} onChange={e => setNewForm(p => ({ ...p, features: e.target.value }))} className="input-glass" /></div>
              </div>
              <div className="flex gap-2"><Button variant="glow" onClick={handleCreate}><Save className="h-4 w-4" /></Button><Button variant="ghost" onClick={() => setShowNew(false)}><X className="h-4 w-4" /></Button></div>
            </div>
          )}
          {plans.map(plan => (
            <div key={plan.id} className="glass-card p-6">
              {editingId === plan.id ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-4">
                    <div><Label>Name</Label><Input value={editForm.plan_name} onChange={e => setEditForm(p => ({ ...p, plan_name: e.target.value }))} className="input-glass" /></div>
                    <div><Label>Price</Label><Input type="number" value={editForm.price} onChange={e => setEditForm(p => ({ ...p, price: +e.target.value }))} className="input-glass" /></div>
                    <div><Label>Features</Label><Input value={editForm.features} onChange={e => setEditForm(p => ({ ...p, features: e.target.value }))} className="input-glass" /></div>
                  </div>
                  <div className="flex gap-2"><Button variant="glow" onClick={handleSave}><Save className="h-4 w-4" /></Button><Button variant="ghost" onClick={() => setEditingId(null)}><X className="h-4 w-4" /></Button></div>
                </div>
              ) : (
                <div className="flex justify-between items-center">
                  <div>
