...
import { LogOut, Plus, Sparkles, TrendingUp, MessageSquare, Settings, Trash2, Eye, Zap, BarChart3, GitCompare, Bell, Users } from "lucide-react";
      ...
      <main className="container mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold mb-2 text-foreground">Welcome to Recall</h1>
          ...
          {selectedBrand ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Recall Score */}
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="bg-card border border-border rounded-lg p-6">
                  <div className="flex items-center gap-4">
                    <div className="relative w-20 h-20">
                      <svg className="w-full h-full transform -rotate-90">
                        <circle cx="40" cy="40" r="32" fill="none" stroke="hsl(var(--muted))" strokeWidth="6" />
                        <circle cx="40" cy="40" r="32" fill="none" stroke="hsl(var(--primary))" strokeWidth="6" strokeLinecap="round"
                          strokeDasharray={`${selectedBrand.recall_score * 2.01} 201`} />
                      </svg>
                      <span className="absolute inset-0 flex items-center justify-center text-xl font-bold">{selectedBrand.recall_score}</span>
                    </div>
                    <div>
                      <h3 className="font-semibold text-lg text-foreground">Recall Score</h3>
                ...
                {/* Score Breakdown */}
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="bg-card border border-border rounded-lg p-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-foreground">{selectedBrand.relevance_score}%</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-foreground">{selectedBrand.clarity_score}%</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-foreground">{selectedBrand.authority_score}%</span>
                      </div>
                ...
                {/* Actions */}
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="bg-card border border-border rounded-lg p-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Button variant="outline" size="sm" className="w-full" onClick={() => navigate("/simulator")}>
              ...
              {selectedBrand.is_optimized && selectedBrand.ai_summary && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-lg p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="font-semibold text-lg text-foreground">AI-Optimized Brand Summary</h3>
                  </div>
                  ...
                  {selectedBrand.ai_recommendation_triggers && (
                    <div className="mt-6">
                      <h4 className="font-semibold mb-3 text-foreground">Recommendation Triggers</h4>
                  ...
                  {selectedBrand.ai_example_snippets && (
                    <div className="mt-6">
                      <h4 className="font-semibold mb-3 text-foreground">Example AI Answer Snippets</h4>
                      <div className="space-y-3">
                        {JSON.parse(selectedBrand.ai_example_snippets).map((snippet: { query: string; answer: string }, i: number) => (
                          <div key={i} className="p-4 rounded-lg bg-secondary border border-border">
                            ...
                            <p className="text-sm text-foreground">{snippet.answer}</p>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            /* Empty State */
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-card border border-border rounded-lg p-12 text-center">
              <div className="max-w-md mx-auto">
                <div className="w-16 h-16 rounded-lg bg-primary/10 flex items-center justify-center mx-auto mb-6">
                  <TrendingUp className="h-8 w-8 text-primary" />
                </div>
                <h2 className="text-2xl font-bold mb-3 text-foreground">Set Up Your First Brand</h2>
