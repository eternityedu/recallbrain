    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="border-b border-glass-border bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <span className="font-display text-xl font-bold glow-text">{isAdmin ? "Support Messages" : "Chat with Support"}</span>
        </div>
      </header>
      <main className="flex-1 container mx-auto px-4 py-4 max-w-2xl flex flex-col">
        <div className="flex-1 glass-card p-4 overflow-y-auto space-y-3 mb-4" style={{ maxHeight: "calc(100vh - 200px)" }}>
          {relevantMessages.length === 0 ? (
            ...
            relevantMessages.map(msg => (
              <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.sender_id === user?.id ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] p-3 rounded-xl ${msg.sender_id === user?.id ? "bg-primary text-primary-foreground" : "bg-muted/50"}`}>
        ...
        <div className="flex gap-2">
          <Input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSend()} placeholder="Type a message..." className="input-glass" />
          <Button variant="glow" onClick={handleSend} disabled={sending || !input.trim()}><Send className="h-4 w-4" /></Button>
        </div>
      </main>
    </div>
  );
